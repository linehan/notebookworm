...
var is_exact = document.getElementById("is_exact");
var is_smooth = document.getElementById("is_exact");
var selectn = document.getElementById("selectn");

searchbutton.addEventListener("click", search_fn);
searchbox.addEventListener("keyup", function(event) {
        var len = this.value.trim().split(" ").length;

        if (this.value.indexOf("*") !== -1) {
                selectn.value = 5;
        } else if (len != selectn.value) {
                selectn.value = len;
        }
        if (event.keyCode === 13) {
                search_fn();
...
function show_results(json)
{
	...
	str += "<table><tr><th>id</th><th>year</th><th>country</th><th>ngram</th><th>frequency</th></tr>";
	...
	for (var i=0; i<237; i++) {
        ...
        for (i=0; i<json.results.length; i++) {
                ...
                str += "<tr data-text-id='"+json.results[i].text_id+"'><td>"+json.results[i].text_id+"</td><td>"+json.results[i].text_year+"</td><td>"+json.results[i].text_country+"</td><td>\""+json.results[i].gram+"\"</td><td>"+json.results[i].freq+"</td></tr>";
...
function search_fn()
{
        ...
        //var mode = (document.getElementById("is_smooth").checked) ? "smooth" : "normal";
        //var exact = (document.getElementById("is_exact").checked) ? "1" : "0";
        ...
        search_req = ajax("/beta/constitutions/server.php", {
                "data":"search="+search+"&n="+n,//"&mode="+mode+"&exact="+exact,
                "method":"POST",
                "success":function(data, xhr) {
                        show_results(JSON.parse(data));
                        document.getElementById("term1").innerHTML = search;
                        document.getElementById("term2").innerHTML = search;
                },
        });
}
